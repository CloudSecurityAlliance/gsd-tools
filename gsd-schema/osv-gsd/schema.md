---
layout: article
title: Open Source Vulnerability format GSD subtype
description: "OSV-GSD schema spec."
permalink: /
aside:
  toc: true
...
**Version 1.0.1 (May 15, 2023)**

Original authors:
- Oliver Chang (ochang@google.com)
- Russ Cox (rsc@google.com)
- Kurt Seifried (kseifried@cloudsecurityalliance.org)
...
The `schema_type` field is used to indicate which schema is explicitly in use. 
This can help consumer applications
decide how to import the data for their own systems and offer some protection
against future breaking changes. The value should be a string matching "osv-gsd"
if it is present.

## osv_gsd_schema_version field
...
The `osv_gsd_schema_version` field is used to indicate which version of the OSV-GSD schema
...
no leading "v" prefix. The value must be specified. Clients can assume that new minor and patch
...
The `relationships` field gives a list of IDs of closely related vulnerabilities, such
as the same problem in alternate ecosystems and includes relationship data. The type is an enum list limited to:
ALIAS, CAUSES, CAUSED_BY, COMMON_NAME, DUPLICATED_BY, DUPLICATE_OF, INCOMPLETE_FIX_FOR, INSUFFICIENT_FIX_OF, RELATED.
...
| `EPSS` | An EPSS value representing the epss score using a version of the [Exploit Prediction Scoring System](https://www.first.org/epss/) (e.g.`"0.01305"`). Please note that the percentile portion of the score is explicitly ignored to prevent unneeded file changes.|
...
- `TIMESTAMP`: The versions `introduced` and `fixed` are time based versions. 
Ranges listed with type `TIMESTAMP` should not overlap: since TIMESTAMPS should have strict
linear ordering, it is always possible to simplify to non-overlapping ranges.
